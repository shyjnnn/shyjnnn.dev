...
- `use client`
  - 서버와 클라이언트 컴포넌트 모듈 그래프 사이의 [경계를 선언](https://react.dev/reference/react/use-client)하는 컨벤션이다.
...
- next.js의 SSR에서 백엔드 리소스에 접근을 하려면, 최상위 페이지에서 `getServerProps` 같은 함수를 사용하거나, 세부적인 컴포넌트 단위에서 백엔드 리소스에 접근하기 어렵다.
